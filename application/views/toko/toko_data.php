                                ...
                                foreach ($toko as $sup) { ?>
                                    <tr>
                                        ...
                                        <td style="text-align: center;width:200px;">
                                            <a onclick="editData(<?= $sup->toko_id; ?>)" style="margin-bottom:4px;color:white;" class="btn btn-primary btn-sm"><i class="fa fa-edit"></i></a>
                                            <form action="<?= site_url('toko/delete') ?>" method="POST" class="d-inline">
                                                <input type="hidden" name="toko_id" value="<?= $sup->toko_id; ?>">
...
<script>
    ...
    function tambahData() {
        ...
        $("#myModal").find('.modal-title').text('Add toko');
        $("#myModal").modal('show', {
            backdrop: 'true'
        });
                ...
                $.ajax({
                    ...
                    success: function(result) {
                        if (result == "ADA") {
                            ...
                            $('#message').html("");
                            $('.nama-ada').hide();
                            $('#formku').attr("action", "<?= site_url('toko/save') ?>");
                            $('#formku').submit();
                        }
                    }
                });
            } else {
                $('#formku').attr("action", "<?= site_url('toko/update') ?>");
                $('#formku').submit();
            }
        };
    };
    ...
    function editData(toko_id) {
        ...
        $.ajax({
            ...
            success: function(result) {
                ...
                $('#toko_id').val(result['toko_id']);

                $("#myModal").find('.modal-title').text('Edit toko');
                $("#myModal").modal('show', {
                    backdrop: 'true'
                });
            }
